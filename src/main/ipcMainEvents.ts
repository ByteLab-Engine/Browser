...
const bytelabAutoLauncher = new AutoLaunch({
  name: 'ByteLab',
});
      ...
      return new Promise((resolve, reject) => {
        ...
        switch (args.key) {
          ...
          case 'application.launchAtStartup':
            args.value === true
              ? bytelabAutoLauncher.enable().catch(() => {
                  reject(
                    new Error(`Couldn't enable ByteLab Browser at startup.`)
                  );
                  rejected = true;
                })
              : bytelabAutoLauncher.disable().catch(() => {
                  reject(
                    new Error(`Couldn't disable ByteLab Browser at startup.`)
                  );
                  rejected = true;
                });
            break;
